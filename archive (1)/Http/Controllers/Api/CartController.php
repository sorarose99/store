...
class CartController extends Controller
{
    ...
    public function update(Request $request, $cartItemId)
    {
        ...
        if (! isset($cart[$cartItemId])) {
            ...
            $result = $this->remove($cartItemId, false);
        } else {
    ...
    public function applyCoupon(Request $request)
    {
        ...
        if (! $coupon) {
            ...
            return response()->json([
                'success' => false,
                'message' => __('messages.coupon_usage_exceeded'),
            ], 422);
        } else {
            $summary = $this->cartService->summary();
            ...
            if ($discount <= 0) {
                return response()->json([
                    'success' => false,
                    'message' => __('messages.coupon_not_applicable'),
                ], 422);
            } else {
    ...
    public function checkout()
    {
        ...
        foreach ($cart['items'] as $item) {
            ...
            if (empty($item['breakdown']) || $totalBreakdownQty != $item['quantity']) {
                return response()->json([
                    'success' => false,
                    'message' => __('messages.please_distribute_sizes_before_checkout', [
                        'product' => $item['name'],
                    ]),
                ], 422);
            }
     ...
     */
    public function updateBreakdown(Request $request, $productId)
    {
        $request->validate([
        ...
        if ($totalQty > $cart[$productId]['quantity']) {
            return response()->json([
                ...
                'message' => __('messages.total_distributed_exceeds_required', [
                    'totalQty' => $totalQty,
                    'requiredQty' => $cart[$productId]['quantity'],
                ]),
            ], 422);
        }

        // التخزين
        $this->cartService->updateBreakdown($productId, $request->breakdown);
