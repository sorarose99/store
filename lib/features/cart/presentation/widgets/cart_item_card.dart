...
class CartItemCard extends StatelessWidget {
      ...
      child: Column(children: [
        Padding(
          ...
          child: GestureDetector(
            ...
            onTap: () {
              print(
                  'CartItemCard onTap: item.id=${item.id}, item.slug=${item.slug}, item.productId=${item.productId}');
              final identifier =
                  item.slug.isNotEmpty ? item.slug : item.productId;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!item.isAvailable) ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                color: context.errorColor, width: 0.5),
                          ),
                          child: Text(
                            'out_of_stock'.tr(),
                            style: TextStyle(
                      ...
                      if (item.options['delivery_type'] != null) ...[
                        _buildDeliveryBadge(
                            context, item.options['delivery_type'].toString()),
                        SizedBox(height: 4.h),
                      ],
                ...
                SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (item.breakdown.isEmpty || item.breakdown.length == 1)
                        Container(
                          height: 28.h,
                          decoration: BoxDecoration(
                            border: Border.all(
        ...
        if (!item.isAvailable)
          Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: 10.5.sp,
                color: context.backgroundColor, // White text on teal background
                fontWeight: FontWeight.bold,
                fontFamily: 'Tajawal',
              ),
  ...
  Widget _buildDeliveryBadge(BuildContext context, String type) {
    String label = '';
    Color color = context.textGrey;
    Color bg = context.border.withValues(alpha: 0.1);

    if (type == 'fast') {
      label = 'fast_shipping'.tr();
      color = AppColors.accent; // gold/orange
      bg = AppColors.accent.withValues(alpha: 0.1);
    } else if (type == 'free') {
      label = 'free_shipping'.tr();
      color = context.primaryColor; // green/teal
      bg = context.primaryColor.withValues(alpha: 0.1);
    } else {
      label = 'standard_shipping_normal'.tr();
      color = context.textMid;
      bg = context.border.withValues(alpha: 0.2);
    }
